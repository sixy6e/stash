...
;change file name outputs, including the mask output
...
;files=FILE_SEARCH('Pro3','2011*9384_Therm_rfl.hdr',COUNT=numfiles)
;files=FILE_SEARCH('Pro3','2011*9384_rfl.hdr',COUNT=numfiles)
files=FILE_SEARCH('Storage01','*9684_Therm_kelvin.hdr',COUNT=numfiles)
;files=FILE_SEARCH('Storage01','*9384_rfl.hdr',COUNT=numfiles)
...
;tile_evf_fname = 'D:\Data\Imagery\Landsat\WRS2_DescendingTiles\9384_WGS84_UTMz55S_.evf'
tile_evf_fname = 'D:\Data\Imagery\Landsat\WRS2_DescendingTiles\Projected\9684_UTMz54s_.evf'
...
envi_evf_info, tile_evf_id, num_recs = Tile_num_recs, $
  data_type = Tile_data_type, projection = Tile_projection, $
...
Tile_rmvSHP = FILE_BASENAME(Tile_layer_name, '.shp') + 'Therm' + '.roi'

;remove the 'Layer: ' from the front of the file name
Tile_roi_out_name = STRMID(Tile_rmvSHP, 7)

;-----------------------------------------------------------------------------------------------------------------------------
;add 'therm' when processing the thermal band, ie. Therm_Tile_Mask
;mask_out_name = 'rfl_Tile_mask'
mask_out_name = 'Therm_Tile_mask'
...
pos=[0]
;bname = ['Band 1', 'Band 2', 'Band 3', 'Band 4', 'Band 5', 'Band 7']
bname = 'Band 6'

ENVI_DOIT, 'ENVI_MASK_APPLY_DOIT', DIMS=Tile_subs_roi_dims, fid=fid, m_fid=m_fid, m_pos=0, $
  VALUE=0, OUT_NAME=file_out_name, r_fid=r_fid, pos=pos, out_bname=bname
