pro batch_mask_and_subset_via_evf_V2
...
;Doesn't quite work for vector files that have multiple records
;The subset is fine, as it will just find the bounding co-ordinates
;For the mask,  it will read the vertices for a record. If there is more than one record,
;it will overwrite the varible and store only the vertices for the last record.
...
;create the array to hold the extents of the vector file
;there should be only 4 values, North, South, East, West
extents = dblarr(4,1)
for e=0, 3 do extents[e,0] = (*subs_evf_id).corners[e,0]

;Create an array listing the four-pair sets of co-ordinates
bounding_rectangle = [ [extents[0,0], extents[2,0]],$
                       [extents[1,0], extents[2,0]],$
                       [extents[1,0], extents[3,0]],$
                       [extents[0,0], extents[3,0]] ]
...
;Mask_rmvSHP = FILE_BASENAME(Mask_layer_name, '.shp') + '_.roi'
...
;mask_out_name = '9293_84overlap_mask'
;file_out_name = fname + '_CIAoverlap_subs'
mask_out_name = '9293_84overlap_Therm_mask'
file_out_name = fname + '_CIAoverlap_subs'

;convert the map co-ordinates to image co-ordinates
