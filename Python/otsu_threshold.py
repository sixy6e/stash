    ...
    if Fast:
        if (len(dims) == 3):
            ...
            for b in range(bands):
                ...
                h = IDL_Histogram(img, locations='loc', omin='omin', binsize=Binsize, max=Max, min=Min, nbins=Nbins)
                hist = h['histogram']
                omin = h['omin']
                loc  = h['loc']
            ...
            h = IDL_Histogram(image, locations='loc', omin='omin', binsize=Binsize, max=Max, min=Min, nbins=Nbins)
            hist = h['histogram']
            omin = h['omin']
            loc  = h['loc']
            binsz = numpy.abs(loc[1] - loc[0])
        ...
        if (len(dims) == 3):
            ...
            for b in range(bands):
                img = image[b].flatten()
                h = IDL_Histogram(img, reverse_indices='ri', omin='omin', locations='loc', binsize=Binsize, max=Max, min=Min, nbins=Nbins)
            ...
            h = IDL_Histogram(image, reverse_indices='ri', omin='omin', locations='loc', binsize=Binsize, max=Max, min=Min, nbins=Nbins)
