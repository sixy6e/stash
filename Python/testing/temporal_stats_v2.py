...
def temporal_stats(infile, outfile, file_driver):
    ...
    all_data = iobj.ReadAsArray()
    output_data = numpy.zeros((outnb, lines, samples), dtype='float32')
    ...
    for tile in tiles:
        ...
        subset = all_data[:,ystart:yend,xstart:xend]
        ...
        output_data[:,ystart:yend,xstart:xend] = stats
    ...
    for i in range(stats.shape[0]):
        outband = outDataset.GetRasterBand(i+1)
        outband.WriteArray(output_data[i])
        outband.SetNoDataValue(noData)
        outband.FlushCache()
        outband = None
    outDataset = None
