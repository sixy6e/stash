    ...
    '''Creates a 2D array of topographic shadow.
       ...
       Args:
           ...
           bitpos: The bit position for specifying the resulting mask (Default
                   value is 14).
           lat: A 2D array of lattitudes in degrees.
           sensor_azi_angle: A 2D array of sensor azimuthal angles in degrees.
           sensor_view_angle: A 2D array of sensor view angles in degrees.
           solar_azi: A 2D array of solar azimuth angles in degrees.
           solar_zen: A 2D array of solar zenith angles in degrees.
...
#--------------Processing Here-------------------------------
    ...
    if (d_xoff > dem_cols) | (d_yoff > dem_rows):
        ...
        The interpolation method I'm using fpr reprojecting the DEM is
        Bilinear. Dealing with DEM's it is either Bilinear or cubic, as the
        results are more visually appealing. For the sake of preserving pixel
        values we could just use nearest neighbour, which should preserve the
        values. Thereby allowing no-data values to be explicitly searched for.
